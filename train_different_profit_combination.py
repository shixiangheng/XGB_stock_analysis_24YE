...
This script try different profit/loss combination points to determine profit percentage and volatility
...
if __name__ == "__main__":
    symbol = 'TQQQ'
    years_in_sample = 1  # Number of years in one sample
    n_samples = 10  # Number of samples
    profit_to_buy_options = [-2,-3, -5,-10,-15,-20,-50]  # Example list of loss percentages to trigger a buy
    profit_to_sell_options = [10,20,30, 40, 50,55,60]  # Example list of profit percentages to trigger a sell

    full_data = fetch_data(symbol, '2019-01-01', '2024-11-01')
    ...
    for profit_to_buy in profit_to_buy_options:
        for profit_to_sell in profit_to_sell_options:
            ...
            for i in range(n_samples):
                ...
                print(f"sell at {profit_to_buy}% Sell at {profit_to_sell}% - Sample {i+1}: Final Portfolio Value: ${final_portfolio_value:.2f}, Profit: {profit:.2f}%")

            avg_profit = np.mean(sample_profits)
            profit_volatility = np.std(sample_profits)
            results.append([profit_to_buy, profit_to_sell, avg_profit, profit_volatility])

    # Convert results to DataFrame
    results_df = pd.DataFrame(results, columns=['Profit to Buy', 'Profit to Sell', 'Average Profit', 'Profit Volatility'])

    # Export DataFrame to Excel
    results_df.to_excel('trading_strategy_results.xlsx', index=False, engine='xlsxwriter')
